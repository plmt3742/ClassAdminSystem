...
import { ArrowLeft, Check, ChevronRight, Search, ShieldAlert, Undo2, X } from "lucide-react"

interface Card {
  id: string; name: string; studentId: string; status: string
  gpa: number; sScore: number
  sem1Gpa: number | null; sem2Gpa: number | null; totalScore: number | null
  autoGpa: number; autoS: number
...
export default function ReviewSPage() {
  ...
  const isLearningCommittee =
    session?.user?.role === "admin" || (session?.user?.tags ?? []).includes("学习委员")

  useEffect(() => {
    if (status !== "authenticated") return
    fetch("/api/zongce/review-s")
      .then(async res => {
  ...
  const handleQuickApprove = async (c: Card) => {
    ...
    if (!confirm(`确认通过 ${c.name}（${c.studentId}）的 S 学习成绩？`)) return
    setApprovingId(c.id)
  ...
  const reload = () => {
    fetch("/api/zongce/review-s").then(r => r.json()).then(d => setCards(d.cards || [])).catch(() => {})
  }

  const visible = useMemo(() => {
    let list = cards
    if (filter !== "all") list = list.filter(c => filter === "failed" ? (c.failedCount > 0 || c.failedPolicyCount > 0) : c.status === filter)
    ...
    return (
      <main className="zc-wrap">
        ...
        <div className="card" style={{ textAlign: "center", padding: "60px 40px", background: "#fff" }}>
          ...
          <p style={{ color: "#7A8A94", fontSize: ".88rem" }}>仅学习委员或管理员可查看 S 学习成绩审核</p>
        </div>
      </main>
    )
  }
  ...
  const mobileView = (
    <div className="m-page-root">
      <header className="m-topbar">
        ...
        <span className="m-title">S 成绩审核<small>REVIEW</small></span>
        <span className="m-year">{year}</span>
      </header>

      {/* 状态统计条：四状态计数（绿/琥珀/红/灰） */}
        ...
        {mList.length === 0 ? (
          ...
          <div className="m-rlist">
            {mList.map((c, i) => {
              ...
              const row = (
                <>
                  ...
                  <span className="body">
                    ...
                    <span className="rv" style={{ color: "#4A7C96", marginTop: 2 }}>
                      GPA {c.gpa.toFixed(2)} · S {c.sScore.toFixed(2)} · 挂科 {c.failedCount}{c.failedPolicyCount > 0 ? `（形策 ${c.failedPolicyCount}）` : ""} · 已填 {c.filled}/{c.total}
                    </span>
                  </span>
              ...
              return clickable
                ? <Link key={c.id} href={`/zongce/review/${c.sectionId}?from=s`} className="m-rrow fx-item" style={{ animationDelay: `${i * 40}ms` }}>{row}</Link>
      ...
      <div className="m-foot" style={{ textAlign: "center", padding: "22px 16px 8px", fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--color-muted-light)", letterSpacing: ".08em" }}>
        各板块待审核提交在此汇总 · 通过后分数即时生效<br /><b style={{ color: "var(--color-muted)" }}>S 成绩审核</b> · {year} 学年
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      <div className="eyebrow">学习成绩审核</div>
      <h1 className="display" style={{ display: "block", marginBottom: 20 }}>S 审核仪表盘</h1>
      ...
      {!loaded ? (
        ...
        <>
          ...
          <div className="stats-row" style={{ marginBottom: 16 }}>
            ...
            <div className="stat"><div className="num" style={{ color: "#C4615A" }}>{cards.reduce((s, c) => s + c.failedCount, 0)}</div><div className="lbl">挂科总数</div></div>
            <div className="stat"><div className="num" style={{ color: "#C7924B" }}>{cards.reduce((s, c) => s + c.failedPolicyCount, 0)}</div><div className="lbl">形策挂科</div></div>
            <div className="stat"><div className="num" style={{ color: "#3D5A6E" }}>{cards.reduce((s, c) => s + c.repeatCount, 0)}</div><div className="lbl">重修课程</div></div>
            ...
            {visible.map(c => {
              ...
              return (
                <div key={c.id} className="rv-row" style={{ opacity: isActive ? 1 : .62 }}>
                  <Link href={c.sectionId ? `/zongce/review/${c.sectionId}?from=s` : "#"} className="rv-left" style={c.sectionId ? {} : { cursor: "default", pointerEvents: "none" }}>
                    ...
                    <div className="rv-badges">
                      {c.failedCount > 0 && <span className="rv-badge fail">挂科 {c.failedCount}</span>}
                      {c.failedPolicyCount > 0 && <span className="rv-badge pol">形策 {c.failedPolicyCount}</span>}
                      {c.repeatCount > 0 && <span className="rv-badge rep">重修 {c.repeatCount}</span>}
                    </div>
                    <div className="rv-data">
                      <div className="rv-d"><div className="v">{c.gpa > 0 || c.totalScore != null ? c.gpa.toFixed(2) : "—"}</div><div className="k">GPA</div></div>
                      <div className="rv-d"><div className="v">{c.gpa > 0 || c.totalScore != null ? c.sScore.toFixed(2) : "—"}</div><div className="k">S 得分{c.totalScore != null && Math.abs(c.totalScore - c.autoS) > 0.01 ? "·手填" : ""}</div></div>
                      <div className="rv-d"><div className="v">{c.filled}/{c.total}</div><div className="k">已填</div></div>
                    </div>
                  </Link>
