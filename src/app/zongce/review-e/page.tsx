...
import { ArrowLeft, Check, ChevronRight, Heart, Search, ShieldAlert, Undo2, X } from "lucide-react"
...
export default function ReviewEPage() {
  ...
  const isReviewer = session?.user?.role === "admin" || (session?.user?.tags ?? []).includes("组织委员")

  useEffect(() => {
    if (status !== "authenticated") return
    fetch("/api/zongce/review-e")
      .then(async res => {
  ...
  const handleQuickApprove = async (c: Card) => {
    ...
    if (!confirm(`确认通过 ${c.name}（${c.studentId}）的 D 社会实践？`)) return
    setApprovingId(c.id)
  ...
  const reload = () => {
    fetch("/api/zongce/review-e").then(r => r.json()).then(d => setCards(d.cards || [])).catch(() => {})
  }
    ...
    return (
      <main className="zc-wrap">
        ...
        <div className="card" style={{ textAlign: "center", padding: "60px 40px", background: "#fff" }}>
          ...
          <p style={{ color: "#7A8A94", fontSize: ".88rem" }}>仅组织委员或管理员可查看 D 社会实践审核</p>
        </div>
      </main>
    )
  }
  ...
  const mobileView = (
    <div className="m-page-root">
      <header className="m-topbar">
        ...
        <span className="m-title">E 社会实践<small>REVIEW</small></span>
        ...
        {mList.length === 0 ? (
          ...
          <div className="m-rlist">
            {mList.map((c, i) => {
              ...
              return clickable
                ? <Link key={c.id} href={`/zongce/review/${c.sectionId}?from=e`} className="m-rrow fx-item" style={{ animationDelay: `${i * 40}ms` }}>{row}</Link>
      ...
      <div className="m-foot" style={{ textAlign: "center", padding: "22px 16px 8px", fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--color-muted-light)", letterSpacing: ".08em" }}>
        各板块待审核提交在此汇总 · 通过后分数即时生效<br /><b style={{ color: "var(--color-muted)" }}>E 社会实践</b> · {year} 学年
      </div>
    </div>
  )
  ...
  return (
    <>
      {mobileView}
      <div className="reviewE-desktop">
      <main className="zc-wrap">
      <button className="btn-ghost" onClick={() => router.push("/zongce")} style={{ marginBottom: 20 }}>
        <ArrowLeft size={14} /> 返回
      </button>
      <div className="eyebrow">社会实践审核</div>
      <h1 className="display" style={{ display: "block", marginBottom: 20 }}>E 审核仪表盘</h1>
      ...
      {!loaded ? (
        ...
        <>
            ...
            {visible.map(c => {
              ...
              return (
                <div key={c.id} className="rv-row" style={{ opacity: isActive ? 1 : .62 }}>
                  <Link href={c.sectionId ? `/zongce/review/${c.sectionId}?from=e` : "#"} className="rv-left" style={c.sectionId ? {} : { cursor: "default", pointerEvents: "none" }}>
                    ...
                    <div className="rv-data">
                      <div className="rv-d"><div className="v">{c.score != null ? c.score.toFixed(2) : "--"}</div><div className="k">E / 5</div></div>
                    </div>
                  </Link>
          ...
          <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 8, fontSize: ".7rem", color: "#7A8A94" }}>
            <Heart size={13} /> 点击卡片查看学生填写的社会实践信息，核对后通过/退回
          </div>
        </>
      )}
    </main>
