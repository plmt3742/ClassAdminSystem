...
interface Card {
  ...
  score: number | null; absences: number; tardies: number; specialLeaves: number
...
export default function ReviewAPage() {
  ...
  const isMonitor = session?.user?.role === "admin" || (session?.user?.tags ?? []).includes("班长")

  useEffect(() => {
    if (status !== "authenticated") return
    fetch("/api/zongce/review-a")
      .then(async res => {
  ...
  const handleQuickApprove = async (c: Card) => {
    ...
    if (!confirm(`确认通过 ${c.name}（${c.studentId}）的 A 学风考勤？`)) return
    setApprovingId(c.id)
  ...
  const reload = () => {
    fetch("/api/zongce/review-a").then(r => r.json()).then(d => setCards(d.cards || [])).catch(() => {})
  }
    ...
    return (
      <main className="zc-wrap">
        ...
        <div className="card" style={{ textAlign: "center", padding: "60px 40px", background: "#fff" }}>
          ...
          <p style={{ color: "#7A8A94", fontSize: ".88rem" }}>仅班长或管理员可查看 A 学风考勤审核</p>
        </div>
      </main>
    )
  }
  ...
  const mobileView = (
    <div className="m-page-root">
      <header className="m-topbar">
        ...
        <span className="m-title">A 学风考勤<small>REVIEW</small></span>
        ...
        {mList.length === 0 ? (
          ...
          <div className="m-rlist">
            {mList.map((c, i) => {
              ...
              const row = (
                <>
                  ...
                  <span className="body">
                    ...
                    <span className="rv" style={{ color: "#4A7C96", marginTop: 2 }}>{"(c) => `旷课 ${c.absences} · 迟到 ${c.tardies} · 请假 ${c.specialLeaves} · 得分 ${c.score != null ? c.score.toFixed(2) : \"—\"}`"}</span>
                  </span>
              ...
              return clickable
                ? <Link key={c.id} href={`/zongce/review/${c.sectionId}?from=a`} className="m-rrow fx-item" style={{ animationDelay: `${i * 40}ms` }}>{row}</Link>
      ...
      <div className="m-foot" style={{ textAlign: "center", padding: "22px 16px 8px", fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--color-muted-light)", letterSpacing: ".08em" }}>
        各板块待审核提交在此汇总 · 通过后分数即时生效<br /><b style={{ color: "var(--color-muted)" }}>A 学风考勤</b> · {year} 学年
      </div>
    </div>
  )
  ...
  return (
    <>
      {mobileView}
      <div className="reviewA-desktop">
      <main className="zc-wrap">
      <button className="btn-ghost" onClick={() => router.push("/zongce")} style={{ marginBottom: 20 }}>
        <ArrowLeft size={14} /> 返回
      </button>
      <div className="eyebrow">学风考勤审核</div>
      <h1 className="display" style={{ display: "block", marginBottom: 20 }}>A 审核仪表盘</h1>
            ...
            {visible.map(c => {
              ...
              return (
                <div key={c.id} className="rv-row" style={{ opacity: isActive ? 1 : .62 }}>
                  <Link href={c.sectionId ? `/zongce/review/${c.sectionId}?from=a` : "#"} className="rv-left" style={c.sectionId ? {} : { cursor: "default", pointerEvents: "none" }}>
                    ...
                    <div className="rv-data">
                      <div className="rv-d"><div className="v">{c.absences ?? 0}</div><div className="k">旷课</div></div>
                      <div className="rv-d"><div className="v">{c.tardies ?? 0}</div><div className="k">迟到</div></div>
                      <div className="rv-d"><div className="v">{c.specialLeaves ?? 0}</div><div className="k">请假</div></div>
                      <div className="rv-d"><div className="v">{c.score != null ? c.score.toFixed(2) : "--"}</div><div className="k">A / 5</div></div>
                    </div>
                  </Link>
